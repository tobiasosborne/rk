...
- **Extraction layer for PDF payloads** (rk-we5i, PRD C7). A payload whose first five bytes are
  ...
  **extraction**; the sidecar is a derived file and is never itself a citable path. The payload
  must also still satisfy its own adopted `sha256` pin — an extraction chained to a replacement
...
- **[rk-stricter-intended] PDF payloads are matched against a chained extraction layer, never
  ...
  there — see the zero-cost evidence above). Fixtures: `refs-17`/`refs-18`/`refs-19`/`refs-20`.
  **Amended 2026-08-14** (Tier A review P1-1): the resolver also requires the payload to satisfy
...
- **Recognized generators.** Two SHAPES: (a) `src/gates/freshness.ts`'s `GENERATORS` map — pure
  ...
  **How a campaign adopts cards**: run `rk render cards`, which writes every card and upserts one
  ...
  reading a card can never quote an extraction the complete record check refused. The fourth pure
  generator is `notation-macros` (rk-5lzf: `definitions/notation/macros.tex` via
  ...
  so producer and verifier cannot drift; fixture `freshness-16`); it is a function of the snapshot
  alone and wraps its bytes in the `PureRegenResult` `ok` case. (b) `RENDER_SITE_GENERATOR` —
...
| `freshness-16` | **rk-5lzf** — `definitions/notation/macros.tex` (generator `notation-macros`) hand-edited so a blessed macro expands to a different symbol than the register blesses ⇒ ERROR naming file, generator and first differing line |
