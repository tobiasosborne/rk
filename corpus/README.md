...
| `defs-17` | defs | **translation-collision** — two notation shards claim the same `(source-id, their-symbol)` pair (`aav-1309.7495`'s `\epsilon`) ⇒ structural ERROR naming both files | **rk-5lzf** (Tier A, LB5): Gate 1's DRIFT namespace extends to `symbol` and to each source-scoped translation pair. Two register entries claiming one source token contradict each other about what that paper says, and nothing downstream can tell which reading a `status: cited` shard inherited. Scoped BY SOURCE deliberately — two campaign symbols may cite one paper, and one source symbol may recur across papers; only the pair is a claim about a specific paper's specific token. | landed |
| `defs-18` | defs | a translation row with **no `"<quote>"` anchor** on the following line ⇒ ERROR `translation-anchor-missing`, row still counted (`0/1 translations verified`) | **rk-5lzf** (Tier A, LB5): a row states that a named source writes a named symbol for this object; with no anchor there is nothing to byte-verify and the register's central claim rests on a lane's assertion. `\Delta` is exactly the token that is a promise gap in one paper and a spectral gap in another. The denominator is what makes the unanchored row visible rather than absent. | landed |
| `defs-19` | defs | **golden pass** — nested notation shard, `shard_type: notation` orthogonal to `kind: cited` (source/sha256 still bind the MEANING), `class:` in the configured `qpcp.v1` profile, one translation byte-verified at its locus ⇒ zero findings, `1/1 cited shards hash-verified`, `1/1 translations verified` | **rk-5lzf** (Tier A, LB5): the control the three red notation fixtures are read against, and the fixture that pins LB5's actual complaint — an anchored source-symbol occurrence must not stand in for the shard's claimed meaning. Editing `refs/aav-1309.7495/paper.tex` invalidates BOTH the lock pin and the checksums prefix; recompute both or the fixture goes red for the wrong reason. | landed |
| `defs-20` | defs | `translations:` written in the **FRONTMATTER** ⇒ ERROR `translations-in-frontmatter`; the body scan finds zero rows | **rk-5lzf** (Tier A, LB5), the ZERO-ROWS false-green: `parseFrontmatter`'s block-list handling (rk-wc3) joins `- item` lines into one `;`-separated string, so rows written there are destroyed before any check sees them — every translation check then passes vacuously at a perfectly clean `0/0 translations verified`. Same shape as dogfood-2's silent-empty-`deps:` (`linker-29`), same remedy: the vacuous state is made loud instead of read as compliance. | landed |
| `defs-21` | defs | two shards at different depths claiming one flat `id` (`definitions/notation/` and `definitions/legacy/`), defining the gap OPPOSITE ways with different `term`s ⇒ structural ERROR `def-id-collision` on both paths | **rk-5lzf repair wave, B6** (Tier A review 2026-08-20, finding 6). RECURSION MADE THIS REACHABLE: in a flat directory two shards cannot share a stem, so nothing ever needed to check. Every consumer keys a definition by flat id ALONE — Gate 2 check 7 collapses both into one `Set` entry, the live verifier's context returns whichever it saw last, the rendered view shows one and hides the other. Different `term`s on purpose, so the pre-existing term/alias DRIFT check sees nothing. | landed |
| `defs-22` | defs | nested `DAG.md`, `notes-2026-08-20.md`, `_scratch.md` are NOT shards; `def-notes-on-gaps.md` IS ⇒ zero findings, `checked 1/1` | **rk-5lzf repair wave, B6 follow-up**: ONE shared non-shard policy. Before it, recursive Gate 1 skipped only README/INDEX and so parsed `DAG.md`/`notes.md` as shards (three spurious structural missing-frontmatter ERRORs, `checked 4/4`), while Gate 9 skipped README/INDEX/DAG — the same tree, two answers to "is this a shard". Green on purpose: the defect it guards is FALSE errors. `def-notes-on-gaps.md` keeps the rule a PREFIX rule, not a substring one. Mutation-proven red-first (shrinking the policy back to {README, INDEX} fails this fixture on verdict, exit code and coverage). | landed |
| `defs-23` | defs | `kind: cited` notation shard carries a byte-verified meaning passage but no `meaning:` declaration ⇒ structural ERROR `meaning-missing` | **rk-5lzf repair B1**: the shard must state the canonical macro's meaning and separately anchor that meaning to source bytes; an anchor cannot fill an absent declaration. | landed |
| `defs-24` | defs | translation row's quote byte-verifies but omits `their-symbol` ⇒ structural ERROR `translation-symbol-not-in-quote` | **rk-5lzf repair B1**: a genuine irrelevant passage is not evidence that the source writes the token claimed by the row. | landed |
| `defs-25` | defs | translation row names source `bmvz` but anchors a path owned by `aav` in `sources.lock.json` ⇒ structural ERROR `translation-source-path-mismatch` | **rk-5lzf repair B1**: source-id owns the path; bytes from paper A cannot prove an attribution to paper B. | landed |
| `defs-26` | defs | cited notation meaning has no `meaning-anchor:` under `phase: exploration` ⇒ structural ERROR, verdict fail, exit 1 | **rk-5lzf repair B1**: notation-admission provenance is phase-independent and cannot demote during conjecture admission. | landed |
| `defs-27` | defs | notation shard in `promise-gap` declares raw `\epsilon` instead of profile-blessed `\gapfrac` ⇒ structural ERROR `symbol-not-blessed-for-class` | **rk-5lzf repair B2**: the profile owns the canonical macro. A shard cannot turn a source spelling or second macro into campaign notation merely by declaring the right class. | landed |
| `defs-28` | defs | notation shard omits `expansion:` even though it drives generated `macros.tex` ⇒ structural ERROR `expansion-missing` | **rk-5lzf review follow-up**: renderer placeholders remain defense-in-depth, not valid register entries. | landed |
| `defs-29` | defs | `expansion:` contains `\input`, an active TeX file-I/O primitive ⇒ structural ERROR `expansion-unsafe` | **rk-5lzf review follow-up**: expansion is one balanced `\ensuremath{...}` replacement body, not an arbitrary TeX program; semantics remain owned by reviewed `meaning` provenance. | landed |
| `notation-01` | notation | raw source symbols `\epsilon` and `\Delta` used in campaign prose ⇒ 2 structural `unblessed-source-symbol` ERRORs; per-class coverage `0/2` | **rk-5lzf** + repair B2: the qPCP literature overloads these tokens; campaign prose must use the class's blessed macro, while source spellings live in source-scoped translation evidence. | landed |
| `notation-02` | notation | `notation-01` under `phase: exploration` ⇒ byte-identical ERROR, verdict fail, exit 1 | **rk-5lzf** + campaign plan section 2a: LB4 named the failure — "the validity barrier becomes advisory exactly when admission begins" — and phase 3's conjecture lanes run in exploration, so a demotable notation check is unenforced precisely where conjectures are admitted. Only meaningful because the corpus runner now applies the phase matrix (`src/corpus/run.ts`, this bead); before that no fixture could express a phase claim and every `structural: true` was unit-tested only. Mutation-proven: dropping `structural: true` leaves `notation-01` green and turns this fixture red. | landed |
| `notation-03` | notation | the same tracked-symbol usage with NO `conventionProfile` configured ⇒ WARN + failed `0/1` profile prerequisite, verdict pass | **rk-5lzf** + repair B4: zero tracked classes never renders as successful `0/0`; the unconfigured state is distinct from configured-but-unusable and remains visibly unenforced. | landed |
| `notation-04` | notation | golden pass — blessed macros registered in their classes ⇒ `2/2`; a translation row and its quote anchor carry a tracked-and-deliberately-unregistered SOURCE token and are NOT flagged | **rk-5lzf**: the control `notation-01` is read against, and the fixture that pins the two exemptions Gate 9 would be unusable without — recording a foreign convention is the register's whole job. The coverage line still states the `c` token it cannot scan for lexically: a green run reports its reach as honestly as a red one. | landed |
| `notation-05` | notation | overlapping raw `\Delta` appears in campaign prose while a shard registers it in one claiming class ⇒ structural ERROR `unblessed-source-symbol` | **rk-5lzf repair B2**, the review's false-green: lexical context cannot decide promise-gap vs spectral-gap. Raw spellings are legal only in source-scoped translation evidence; campaign prose uses the unique blessed macro. | landed |
| `notation-06` | notation | fully double-quoted campaign sentence carrying raw `\epsilon`, with no adjacent strict source pointer/translation row ⇒ structural ERROR | **rk-5lzf repair B3**: quotation marks alone are not verified source evidence. The former first/last-quote heuristic let any shard sentence or quoted `statement_blessed` bypass Gate 9. | landed |
| `notation-07` | notation | profile declares `notation: complete`, but spectral has no canonical shard ⇒ structural ERROR `canonical-shard-missing`; per-class coverage `1/2` lists every skipped token | **rk-5lzf repair B4**: phase 0c completion is explicit, not inferred from whatever symbols happened to occur. Coverage is registered classes / profile classes, so an untouched class cannot disappear into a pass-shaped `0/0`; per-class skipped lists are deterministic and untruncated. | landed |
...
| `config-06` | config | `conventionProfile: "qpcp.v2"` naming `.rk/conventions/qpcp.v2.json`, which is not present (a well-formed `qpcp.v1.json` IS) ⇒ structural ERROR naming the expected path and what is present; `checked 1/2`, profile unit `0/1` | **rk-5lzf** (Tier A, LB5 of `docs/reviews/2026-08-20-qpcp-plan-tierA-codex.md`): "the profile you configured does not exist" and "you configured no profile" are different facts and must never print the same — the second means Gate 9 has nothing to check against and says so in its own coverage line; the first means the campaign believes it IS being checked and is not. `aism_behavior: n/a` — the convention profile is rk-only. | landed |
| `config-07` | config | `qpcp.v2.json` is `qpcp.v1.json` with the `locality` tracked class deleted and `version` left at `1` ⇒ structural ERROR `class-removed-without-bump` naming the dropped class and both `version` values | **rk-5lzf** (Tier A, LB5): "Letting the register itself declare tracked classes also lets deletion of a class shrink coverage." The profile lives outside the register so a shrink is *visible*; this fixture is the copy-paste-and-forget shape that makes it *blocking*. Without the check, Gate 9 silently stops enforcing `\kloc`/`\qdim` and still reports green. The filename's `v2` is only the reference key — the `version` FIELD is the rule-10 compat counter the check reads, so renaming a file can never launder a shrink. Mutation-proven red-first (relaxing the comparison to `>=` drops the finding and turns this fixture green; reverted after confirming). | landed |
| `config-08` | config | `qpcp.v2` declares a predecessor hash but `qpcp.v1.json` was deleted ⇒ structural ERROR `predecessor-missing` | **rk-5lzf repair B5**: a hash without the exact predecessor bytes cannot establish compatibility history. Successors retain their immediate same-family predecessor. | landed |
| `config-09` | config | `qpcp.v1.json` and `qpcp.v3.json` exist but `qpcp.v2.json` is skipped ⇒ structural ERROR `predecessor-missing` naming v2 | **rk-5lzf repair B5**: every link is `n -> n-1`; skipping a filename version cannot bypass the comparison. | landed |
| `config-10` | config | `renamed.v2.json` is accompanied only by `qpcp.v1.json`, not `renamed.v1.json` ⇒ structural ERROR | **rk-5lzf repair B5**: family name is part of chain identity; a rename cannot launder a shrink. | landed |
| `config-11` | config | `qpcp.v2` pins the original two-class v1 hash, but v1 was rewritten in place to one class ⇒ structural ERROR `predecessor-hash-mismatch` | **rk-5lzf repair B5**: mutating history is not a compat bump. The chain validates exact raw predecessor bytes before comparing classes. | landed |
| `config-12` | config | duplicate `allowed_translations` values accepted by the JSON parser but forbidden by schema `uniqueItems:true` ⇒ structural profile ERROR | **rk-5lzf review follow-up**: runtime validation now matches the schema instead of admitting a shape schema consumers reject. | landed |
...
Totals: 12 config + 29 defs + 7 notation + 46 argument/linker + 22 refs + 24 provenance + 10 runs +
15 report-shards + 12 freshness + 28 reward = **205 fixtures** (reconciled 2026-08-10,
...
+1 refs-20 review repair 2026-08-14; +2 rk-r0j3, +2 rk-yic3 2026-08-14; +12 rk-5lzf `defs-16`..`defs-20`,
`config-06`, `config-07`, `freshness-12`, `notation-01`..`notation-04` 2026-08-20;
+2 the rk-5lzf Tier A repair wave `defs-21`, `defs-22`; +4 repair-B1 `defs-23`..`defs-26`;
+2 repair-B2 `defs-27`, `notation-05`; +1 repair-B3 `notation-06`; +1 repair-B4 `notation-07`;
+4 repair-B5 `config-08`..`config-11`; +3 review follow-ups `config-12`, `defs-28`, `defs-29`
2026-08-21) across the gates named in
