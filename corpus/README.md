...
| `linker-47` | argument/linker | **Check 17, THE red fixture** — `lem-amp` holds at `qdim: poly` and produces `gap: const`; `thm-qpcp` runs at `qdim: const` and consumes `gap: const` + `qdim: const` ⇒ structural ERROR `[regime-unentailed]` naming `lem-amp` and its `qdim` | **The 2026-08-20 Tier A review's own LB2 pair** (docs/reviews/2026-08-20-qpcp-plan-tierA-codex.md): v1's atom-wise matching PASSED this, because the gap atom came from the dependency and the dimension atom from the parent's OWN regime — so a result proved only for polynomial-dimensional qudits silently backed a constant-dimension theorem. Under interval entailment the context's `qdim` interval `['const','const']` is not CONTAINED in the required `['poly','poly']`, so the lemma is never made available and its post never joins the context. Green twin: `linker-49`. | landed |
| `linker-48` | argument/linker | **Check 17, the CAPPED reading** — a lemma needing `qdim: [*, const]` consumed by a theorem whose ambient is `qdim: poly` ⇒ `[regime-unentailed]` | **Codex Tier A review of the convention-profile draft, findings 10-11**: values are INTERVALS and entailment is containment, which is why no key needs a direction flag. The same containment rule that rejects `linker-47`'s afforded-style clash rejects this capped-style one; a per-key polarity flag (the design this replaced) is one more thing that can be set wrongly. | landed |
| `linker-49` | argument/linker | **Check 17 golden pass** — a capped requirement `[*, const]` met by a point context, an enum key matched exactly, and a RANGED own-post `['inv-poly','const']` supplied by a point inside it ⇒ zero findings, exit 0 | **No-false-positive guard** for `linker-47`/`48`/`57`: without it, a checker that rejected everything would pass all three. Each of the three predicates is independently load-bearing — edit any one and this fixture goes red. | landed |
| `linker-50` | argument/linker | **Check 17(a)** — a signature constraining `def-spectator-register`, which no `definitions/*.md` shard defines ⇒ structural ERROR `[dangling-object]` | The object id is taken verbatim from **review LB3**'s bite exploit ("clause (iii) passes after adding a fresh but irrelevant `def-spectator-register`"). A signature naming an unresolvable object is a broken cross-shard reference — the Phase matrix's own fourth structural class. | landed |
| `linker-51` | argument/linker | **Check 17(c), key half** — predicate key `sparsity`, which the convention profile never declares ⇒ structural ERROR `[unknown-key]` | An undeclared key has no declared order, so there is no rule by which it could be compared. Reported as merely "unmet" it would read as a substantive regime clash; ignored, it would let an author write any constraint they liked and have it checked by nothing. | landed |
| `linker-52` | argument/linker | **Check 17(c), value half** — `gap: inverse-polynomial`, the profile's `inv-poly` spelled the way a human writes it ⇒ structural ERROR `[unknown-value]` | The near-miss a substring or fuzzy comparison would silently accept. The vocabulary is CLOSED: an undeclared value has no place in its key's order. BOTH endpoints of an interval are checked, not just the first. | landed |
| `linker-53` | argument/linker | **Check 17 fail-closed** — one trailing comma inside `pre` makes the block unparseable ⇒ structural ERROR `[signature-malformed]`, and NO `[signature-missing]` | **Memo section 6 / review LB6**: a malformed signature is an ERROR, NEVER "no signature". Degraded to "absent" it would skip the entailment check entirely and, under `signatures: required`, be reported under a DIFFERENT name, sending the author to look for a block that is right there. One fault, one finding. | landed |
| `linker-54` | argument/linker | **Check 17 required-ness** — `signatures: "required"`, a `theorem` with no block ⇒ structural ERROR `[signature-missing]`; the `open-problem` beside it produces nothing | The second shard is the DISCRIMINATOR: a fixture asserting only the ERROR would still pass if the check fired on every kind indiscriminately. The `optional` (WARN) and ABSENT (no finding, "not adopted" on the coverage line) states are pinned in `test/gates/linker-signature.test.ts` — a corpus fixture carries one config at a time. | landed |
| `linker-55` | argument/linker | **Check 17's deliberate WARN** — a lemma whose own `post` no route dependency supplies ⇒ `[post-unsupported]` WARN, gate still exits 0 | Not an ERROR, on purpose: a shard's own post may legitimately be supplied by its PROOF rather than by any dependency — that is what proving something means. The signal still ships, because the same shape is what a shard claiming more than its route provides looks like. | landed |
| `linker-56` | argument/linker | **Check 17 is STRUCTURAL** — `linker-47`'s clash in a repo whose `.rk/config.json` sets `phase: "exploration"` ⇒ the same ERROR, undemoted | **Memo section 2a, repair of review LB4**: admission of a conjecture happens DURING exploration — the phase that demotes everything else — so a demotable signature barrier is advisory at precisely the moment it is load-bearing. Harness boundary: `src/corpus/run.ts` calls `gate.run` directly and never passes through `applyPhase`, so this fixture proves the gate's own verdict is phase-independent; the `structural: true` flag itself is pinned in `test/gates/phase-classification.test.ts`'s Check 17 block, same commit (the Phase matrix's three-way mutation-proof rule). | landed |
| `linker-57` | argument/linker | **Check 17 on a POSET key** — a `quasi-poly` ambient consuming a lemma stated under `turing` reductions ⇒ `[regime-unentailed]`; the two are INCOMPARABLE, not ordered | **Codex Tier A review of the profile draft, finding 12**: `reduction` is genuinely not totally ordered (`quasi-poly` is a time bound, `turing` a query-model widening; neither implies the other), and linearising it would make one silently discharge the other — quiet over-acceptance on a validity surface. The symmetric direction is pinned in `test/gates/signature-entail.test.ts`; both share one reachability rule. | landed |
| `linker-58` | argument/linker | **Check 17, an inconsistent interval** — `gap: ['const','inv-poly']` runs backwards in its chain ⇒ structural ERROR `[signature-malformed]` | Classified as a broken VALUE, not a regime clash (**review finding 10**): reporting it as `[regime-unentailed]` would send the author hunting for a dependency that could satisfy something unsatisfiable by construction. The check needs the profile (the order is profile-defined), which is why it lives with the vocabulary stage and not the shape parser; the same rule refuses an interval between two INCOMPARABLE endpoints on a poset key. | landed |
| `linker-59` | argument/linker | **Check 17 contradictory predicates (rk-8805 review B1)** — a parent declares ambient `qdim=const` AND `qdim=poly`, while `lem-poly` requires `qdim=poly` and provides the parent's claimed `gap=const` post ⇒ structural ERROR `[signature-contradictory]` AND `[regime-unentailed]` names `lem-poly` unavailable | Before the repair, context values were stored separately and ANY matching interval entailed, so the contradictory parent unlocked `lem-poly` and received its post. Context predicates are now conjoined: the empty intersection entails nothing, and the dependency post is not added. | repair wave |
| `linker-60` | argument/linker | **Check 17 canonical uniqueness, duplicate form (rk-8805 review B2)** — the same `(object,key,value)` predicate is authored twice ⇒ structural ERROR `[signature-noncanonical]` | Before the repair, per-entry sorting retained both copies: the one-copy and two-copy signatures stated the same `(scope,key)->value-set` but had different canonical bytes, breaking bite's identity premise. Per-scope grouping now deduplicates values before rows are emitted. | repair wave |
| `linker-61` | argument/linker | **Check 17 canonical uniqueness, regrouped form (rk-8805 review B2)** — one object's `gap` and `norm` keys are split across two predicate entries ⇒ structural ERROR `[signature-noncanonical]` | Before the repair, per-entry sorting preserved author-chosen grouping: split and combined forms were mutually strong but serialized differently. Per-scope/per-key grouping now emits one combined predicate in the normal case, so equal value sets have equal canonical bytes. | repair wave |
| `linker-62` | argument/linker | **Check 17 kind evasion (rk-8805 review B3)** — `kind: open-problem`, `status: proved`, no signature under `signatures: required` ⇒ structural ERRORs `[signature-missing]` AND `[kind-status-incoherent]` | Before the repair, required-ness consulted four signed kinds only; kind/status are independent enums, so a proved result could evade the signature barrier by claiming an exempt kind. Required adoption now demands a block for a signed kind OR signed status OR signed af state, and adopted repositories reject rigorous-result statuses on open-problem/obstruction kinds. | repair wave |
...
Totals: 5 config + 15 defs + 62 argument/linker + 22 refs + 24 provenance + 10 runs +
15 report-shards + 11 freshness + 28 reward = **192 fixtures** (reconciled 2026-08-10,
...
+1 refs-20 review repair 2026-08-14; +2 rk-r0j3, +2 rk-yic3 2026-08-14;
+12 rk-8805 2026-08-20; +4 rk-8805 Tier A repair B1-B3 2026-08-21) across the gates named in
...
| `graph/signature-legacy-v2` | `test/graph/signature-graph-schema.test.ts` | **rk-8805, the LEGACY half of the graph `schema_version` 2 -> 3 bump (rule 10).** A document exactly as a pre-rk-8805 producer emitted it — `"schema_version": "2"`, no `signature` anywhere. `acceptGraphDocument` (src/graph/schema-version.ts) reads it, reports `sourceVersion: "2"`, and upgrades it in memory with every node's signature ABSENT — never a fabricated empty one, because "this shard declared no regime" and "this shard declared the empty regime" are different claims. Hand-written and never re-serialized by today's code (which would stamp `"3"`): that is what makes it a legacy artifact rather than a regenerated one. A node carrying a `signature` under a v2 header is refused. | landed |
| `graph/signature-v3` | `test/graph/signature-graph-schema.test.ts` | **rk-8805, the v3 GOLDEN.** Three nodes: one with an interval-valued signature (`qdim: [null,"const"]` capped, a point enum, a ranged post), one with a point-valued one, and one with NO signature at all — absent is a first-class state. Pinned two ways: it must load without upgrade AND serialize byte-identically to its own file bytes, so a drift in the canonicaliser (sorted keys, sorted entries, point-interval collapse) fails here rather than silently changing a document's identity. `validateGraphDocument` names the NODE when a signature is non-canonical, because a signature is the field a person hand-writes and therefore the one they have to go and fix. | landed |
