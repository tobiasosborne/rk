...
| `linker-47` | argument/linker | **Check 17, THE red fixture** — `lem-amp` holds at `qdim: poly` and produces `gap: const`; `thm-qpcp` runs at `qdim: const` and consumes `gap: const` + `qdim: const` ⇒ structural ERROR `[regime-unentailed]` naming `lem-amp` and its `qdim` | **The 2026-08-20 Tier A review's own LB2 pair** (docs/reviews/2026-08-20-qpcp-plan-tierA-codex.md): v1's atom-wise matching PASSED this, because the gap atom came from the dependency and the dimension atom from the parent's OWN regime — so a result proved only for polynomial-dimensional qudits silently backed a constant-dimension theorem. Route-scoped entailment demands a dependency's ENTIRE pre+regime before its post joins the context. Green twin: `linker-49`. | landed |
| `linker-48` | argument/linker | **Check 17, polarity `capped`** — a `qdim_cap: polylog` ambient consuming a lemma that requires `qdim_cap: const` ⇒ `[regime-unentailed]` | **Campaign profile draft `conventions-qpcp-v1-draft.md` 9.1 / D4**: the memo's single-direction rule is unsound for CAPPED constraints — read literally it lets a loose ambient discharge a tight cap. Polarity runs the comparison the other way for capped keys: the context's guarantee must be at least as TIGHT. Separately breakable from `linker-47`: deleting the polarity branch leaves `linker-47` green and turns this red. | landed |
| `linker-49` | argument/linker | **Check 17 golden pass** — the same chain in the regime the amplifier actually holds in, exercising all three polarities at once (`qdim` afforded reaches poly, `qdim_cap` capped is guaranteed tighter than required, `norm` equality matches) ⇒ zero findings, exit 0 | **No-false-positive guard** for `linker-47`/`linker-48`: without it, a checker that rejected everything would pass both red fixtures. Each of the three predicates is independently load-bearing — edit any one and this fixture goes red. | landed |
...
Totals: 5 config + 15 defs + 56 argument/linker + 22 refs + 24 provenance + 10 runs +
...
+10 rk-8805 2026-08-20) across the gates named in
