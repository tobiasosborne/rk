...
| `refs-23` | refs | extraction record whose `statement_range` stops one line before the printed statement's `where` clause — every other clause impeccable, review hash-bound and VALID ⇒ Check 11 ERROR `[statement-range-truncated]` | **rk-nsex** (Tier A), the OMITTED-HYPOTHESIS fixture the campaign memo and review finding LB1 both name: "requiring an anchor on every PRESENT hypothesis cannot detect an OMITTED hypothesis." The hypothesis is not mis-anchored, it is absent, so clauses (a)-(c) all pass; only the range-extent check sees it, via the lowercase continuation line immediately after the range. Documented as a HEURISTIC in `docs/gate-contracts.md` Gate 3 Check 11 clause (f) and at `src/gates/refs-records-verify.ts`'s `extentProblem`, with both its blind spot (truncation at a sentence boundary) and its false-positive mode (a complete statement followed by a lowercase line) stated; its sufficiency is the residual the Tier A review recorded on rk-nsex. Mutation-proven red-first (deleting the extent call returns the tree to exit 0 with `1 reviewed-VALID`; reverted). Green control: `refs-32`. | landed |
| `refs-24` | refs | `status: cited` shard whose citation quote is genuine and byte-verbatim at its locus, joined to a valid record by correct `record_sha256`, but whose `contract:` states more than the record's `statement_blessed` ⇒ Check 12 ERROR `[contract-mismatch]`, coverage `checked 1/1 shard citations`, `0/1 shard-record joins` | **rk-nsex** (Tier A), review finding LB1 verbatim: "A `status: cited` shard can carry one genuine but irrelevant quote and an arbitrary contract." Check 8 passes here — that is the whole point — and the claim the linker propagates rigour through is still stronger than the paper's. The comparison uses Gate 2 Check 9's own `normalizeContract` (exported from `src/gates/linker-graph.ts`, never re-derived). Mutation-proven red-first (removing the contract comparison returns the tree to exit 0 at `1/1 shard-record joins`; reverted). Green control: `refs-32`, identical but for the contract text. | landed |
| `refs-25` | refs | extraction record with no `statement_range` and no hypothesis anchors, beside a valid hash-bound review ⇒ Check 11 ERROR `[zero-anchor-record]`, coverage `1 L1 records / 0 reviewed-VALID / 0 anchors verified` | **rk-nsex** (Tier A), named in the campaign memo's red-fixture list. A record whose whole function is anchoring, anchoring nothing: the reviewer who approved it was reviewing the author's memory of the paper. Reported under its own code rather than as a missing required field, and visible from the other side on the coverage line (in the denominator, contributing to no numerator — L2). Mutation note, stated exactly: the raw VERDICT here is redundantly guarded by the required-field list (disabling the zero-anchor branch still ERRORs, as `[record-malformed]`), so what the check adds — and what this fixture's `message_pattern` actually asserts — is the distinct code, the reason, and the coverage story. | landed |
| `refs-26` | refs | record declaring the `payload_sha256` of revision 1 while `sources.lock.json` adopts revision 2 ⇒ Check 11 ERROR `[stale-record]` (anchors still verify, review still VALID) | **rk-nsex** (Tier A), the memo's "stale extraction" case at the payload layer. The record is internally impeccable and agrees with today's bytes by luck of which lines survived the revision; a v2 that renumbered lines would leave every anchor still "verifying" against a statement the reviewer never read, which is why the check compares against the LOCK and not against the text. Remedy is re-extraction plus re-review, never repinning by hand. Mutation-proven red-first (dropping the payload comparison returns exit 0; reverted). Sibling: `refs-27`. Nearest existing neighbours: `refs-18`/`refs-21`. | landed |
| `refs-27` | refs | PDF source whose lock chain is intact and whose record names the digest of an EARLIER extraction of the same payload ⇒ Check 11 ERROR `[stale-extraction]` | **rk-nsex** (Tier A). Re-extraction silently renumbers a document, so record and sidecar can each be valid and jointly meaningless. Gate 3's own resolver (Check 10) is fully satisfied here — `refs-18`/`refs-19`/`refs-20` test the LOCK's chain to the payload, this tests the RECORD's chain to the lock — so none of them can detect it. Payload and sidecar are `refs-17`'s, reused unchanged. Mutation-proven red-first (dropping the extraction comparison returns exit 0; reverted). | landed |
| `refs-28` | refs | mechanically perfect extraction record with NO review record ⇒ Check 11 ERROR `[review-absent]`, coverage `2 anchors verified` but `0 reviewed-VALID` | **rk-nsex** (Tier A), LB1's headline: anchoring establishes that a record QUOTES the paper correctly, never that its restatement SAYS what the paper says. The card `rk render cards` renders for this record is the NOT ADMISSIBLE stub (no statement, no hypotheses, no restatement), so the unreviewed extraction cannot leak through the artifact agents actually read either. Mutation-proven red-first (treating a missing review as "nothing to check" returns exit 0; reverted). Siblings: `refs-29`, `refs-30`. | landed |
| `refs-29` | refs | two records: one whose review says `verdict: INVALID`, one whose review says VALID while answering `hypotheses_complete: false` ⇒ Check 11 ERRORs `[review-invalid]` and `[review-inconsistent]` | **rk-nsex** (Tier A). One fixture, one rule ("the verdict must be honestly VALID"), two separately breakable halves — dropping only the false-clause test leaves the first red and the second green, which is why both live in one tree. The second is the shape a rubber-stamp takes when the reviewer does the work, records the defect, and ticks the box anyway; a verdict may not be more generous than the reviewer's own clauses. | landed |
| `refs-30` | refs | complete VALID review bound by `card_sha256` to the record as it was BEFORE its `statement_blessed` was edited ⇒ Check 11 ERROR `[review-stale]` | **rk-nsex** (Tier A), the memo's "stale record (`card_sha256` mismatch)". Anchors still verify — the quoted hypothesis never changed; the RESTATEMENT, the part only a human read can check, did. Same staleness class as Gate 8's `claimSha256` (`reward-23`) and the L5 store's `contentHash` (`linker-36`), applied to the artifact agents read. The digest is over CANONICAL bytes, so a reformat does not fire it and a one-word edit always does (`test/gates/canonical-json.test.ts`). Mutation-proven red-first (dropping the digest comparison returns exit 0 at `1 reviewed-VALID`; reverted). | landed |
| `refs-31` | refs | fabricated `statement_verbatim` behind a hash-bound VALID review, in a repo whose `.rk/config.json` declares `phase: exploration` ⇒ Check 11 ERROR, structural, verdict fail | **rk-nsex** (Tier A), review finding LB4: "the validity barrier becomes advisory exactly when admission begins." Checks 11-12 are the first Gate 3 checks in the STRUCTURAL column — admission of a cited result is a phase-independent transaction (campaign memo section 2a) while the rest of Gate 3 stays demotable. Scope, stated rather than implied: the corpus harness calls `gate.run()` directly and never applies `applyPhase`, so this tree proves the ERROR is RAISED (and carried `structural: true`) under a declared exploration phase; that it SURVIVES `applyPhase(findings, "exploration")` unchanged is proven in `test/gates/refs-records.test.ts` and `test/gates/refs-card-join.test.ts`, and the classification is pinned in `test/gates/phase-classification.test.ts`. One fixture tree cannot carry both halves. | landed |
| `refs-32` | refs | complete record + hash-bound VALID review + `status: cited` shard joined by canonical `record_sha256` with a matching contract ⇒ PASS, `1/1 shard citations`, `1 L1 records / 1 reviewed-VALID / 2 anchors verified`, `1/1 shard-record joins` | **rk-nsex** (Tier A) GREEN CONTROL: proves Checks 11-12 do not false-positive on the case they exist to let through, with every counter non-vacuous. Each red sibling is this tree with exactly one thing changed (`refs-23` the range, `refs-24` the contract, `refs-25` the anchors, `refs-26`/`refs-27` the source's age, `refs-28`/`refs-29`/`refs-30` the review binding, `refs-31` the verbatim). Deliberately excludes the generated card and its `.rk/generated.json` entry: the corpus runner runs ONE gate per fixture, so a committed card here would be unchecked output that could rot silently — Gate 7's byte-diff of a `cards-v1` artifact is proven in `test/render/cards.test.ts` and `test/cli-render-cards.test.ts`. | landed |
| `refs-33` | refs | extraction record filed under `refs/records/widget-2026/` declaring `source: widget-2026` whose range, payload hash, quotation, hypothesis and blessed statement all come from a second acquired paper the manifest attributes to `other-2026` ⇒ Check 11 clause (g) ERROR `[source-mismatch]` and the join's consequent `[record-review-unusable]` | **rk-nsex Tier A repair wave, landing-blocker BL1** — the reviewer's own constructed triple, transcribed. Every other clause passes because internal consistency is exactly what a wholesale substitution preserves: the anchors verify against the wrong paper, the verbatim matches the wrong paper's bytes, the payload satisfies the wrong paper's pin, and the review is hash-bound and VALID. Pre-repair this tree emitted ZERO findings at `1/1 shard-record joins` — a cited theorem attributed to a paper that does not contain it. The fix retains `source_id` in `LockEntryFacts` and binds every anchored payload to the record's own source. Mutation-proven red-first (dropping the `checkSourceBinding` call restores exactly that false green; observed and reverted). | landed |
| `refs-34` | refs | the hypothesis is printed on the result's label line and `statement_range` starts on the line BELOW it ⇒ Check 11 clause (f) START ERROR `[range-start-unlabelled]` | **rk-nsex Tier A repair wave, landing-blocker BL2 (START half)** — the reviewer's own example. Pre-repair every clause passed: the verbatim equalled the ranged line, no hypothesis anchor could fall outside a range with no hypotheses, and the old check looked only at what FOLLOWED the range, which was a blank line and `Proof.` — a perfectly well-formed terminator. The extracted `statement_blessed` was therefore an unconditional theorem taken from a conditional one. The START rule is mechanical, not heuristic. Mutation-proven red-first (disabling the START clause restores exit 0 with `1 reviewed-VALID`; reverted). | landed |
| `refs-35` | refs | the statement continues on the next line as `Consequently …` — capitalised, unseparated, and deliberately OUTSIDE the continuation word list ⇒ `[statement-range-truncated]` | **rk-nsex Tier A repair wave, landing-blocker BL2 (capitalised-continuation half)**: the pre-repair test was lexical (lowercase initial, or one of eight words), so any capitalised continuation passed. The replacement is structural — the only accepted terminators are a new labelled block, or a blank line followed by a capitalised non-continuation sentence. The word is chosen outside the list on purpose: a fixture whose defect the list happens to catch would prove the list, not the rule. | landed |
| `refs-36` | refs | the quotable text ends exactly where `statement_range` ends, with no L0 `ends_at_eof` declaration ⇒ `[range-ends-at-eof]` | **rk-nsex Tier A repair wave, landing-blocker BL2 (EOF half)**: pre-repair the scan ran off the end of the line array and returned 'no problem', so every truncated-at-EOF extraction passed by construction — the shape a page-bounded extraction takes when the theorem continues onto a page that was never extracted. The escape hatch is explicit and reviewable: the source's L0 record may declare `ends_at_eof` for that exact label (`refs-27` is the positive control; the declaration is per-label, pinned in `test/gates/refs-records.test.ts`). | landed |
| `refs-37` | refs | `.rk/config.json` `records: "required"`; a `status: cited` shard carrying a genuine citation pair and an arbitrary contract names no extraction record, though a complete reviewed record for that very theorem sits in the repo ⇒ structural ERROR `[record-required]`, coverage `0/1 shard-record joins` | **rk-nsex Tier A repair wave, landing-blocker BL3**: pre-repair this exited 0 with a WARN AND was excluded from the join denominator, so the coverage line read `0/0` — full coverage of a population the check had defined to be empty. Two separately breakable repairs, both asserted here: the required mode, and the denominator. | landed |
| `refs-38` | refs | required mode; `status: proved-mod-audit` shard declaring no `origin:` ⇒ ERROR `[origin-required]` | **rk-nsex Tier A repair wave, BL3 (origin half)**: 'a proved-mod-audit shard without a record or citation produces no finding at all.' rk cannot tell a literature PMA claim from a campaign proof, and the pre-repair gate resolved that by inferring 'campaign' from silence. The repair makes the campaign say which it is rather than inventing a proxy. Denominator correctly `0/0`: an undeclared shard is not yet in the join population, and that it is undeclared IS the finding. | landed |
| `refs-39` | refs | required mode; `status: proved-mod-audit` + `origin: literature` naming no record ⇒ ERROR `[record-required]` | **rk-nsex Tier A repair wave, BL3**: the clause that makes the origin declaration mean something — a literature claim is held to exactly the cited standard (same code, same denominator, same structural classification), per the campaign memo's 'it is a literature claim, not a campaign proof'. | landed |
| `refs-40` | refs | no `.rk/config.json`, so the default `records: "legacy"`; the byte-identical tree to `refs-37` ⇒ WARN `[record-absent]`, exit 0, coverage `0/1 shard-record joins` | **rk-nsex Tier A repair wave, BL3 — THE LEGACY CONTROL**, and the guard against a future change quietly removing the migration path. Legacy is a migration setting, not a resting state: what it no longer does is HIDE the population, so the backlog is a number a campaign watches go to zero before flipping to `required`. The second shard (`origin: campaign`) proves that half stays silent and contributes nothing to the denominator. | landed |
| `refs-41` | refs | one extraction record changes a reviewed integer from `9007199254740992` to `9007199254740993`, while a second repeats `statement_blessed`; both retain the digest `JSON.parse` would have produced ⇒ structural `[record-malformed]` ERRORs and `0/2 shard-record joins` | **rk-nsex Tier A repair wave, BL5** — canonical hashing was lossy before it saw values: the unsafe-number pair parses to one double and duplicate keys collapse to the last value, so an old review and shard hash carried forward to raw documents nobody reviewed. `scanCanonicalJson` now rejects either form before hashing. Mutation-proven by bypassing the scan, which restores zero findings and `2/2` joins; reverted. | landed |
| `refs-42` | refs | an L1 record and its review carry extra properties at every closed nesting level ⇒ structural `[record-malformed]` and `[review-malformed]`, `0 reviewed-VALID` | **rk-nsex Tier A repair wave, BL6 — `additionalProperties: false` in runtime, not only on paper.** Covers L1 top-level + hypothesis, and review top-level + reviewer + checked + clause key sets. Before repair all six extras were ignored and the record became usable. Mutation-proven by bypassing `exactL1Problems`/`exactReviewProblems`, which restores zero findings and `1 reviewed-VALID`; reverted. | landed |
| `refs-43` | refs | malformed L0 SHA-256 fields, non-string `objects`/`results` items, an extra property, plus a second L0 whose `source` differs from its directory ⇒ `[record-malformed]` + `[record-misfiled]` | **rk-nsex Tier A repair wave, BL6 — the L0 branch was only subset-validated.** Before repair both L0 records were accepted; exact runtime enforcement now checks SHA shape, array item types, the closed top-level key set, optional-field shapes, and source-directory equality. Mutation-proven by bypassing `exactL0Problems` and the filing comparison, which restores zero findings; reverted. | landed |
...
| `freshness-12` | freshness | reviewed extraction record and a rendered card on disk, with NO `.rk/generated.json` at all ⇒ ERRORs `[card-unadopted]` + `[card-undeclared]`, coverage names the count | **rk-nsex Tier A repair wave, landing-blocker BL4**: 'a valid record with no manifest … yields clean 0/0'. Gate 7's whole-mechanism early return is the right answer to 'has this repo adopted freshness?' and the wrong answer to 'is the card an agent will read checked by anything?' — regenerate-and-diff only verifies what the manifest DECLARES, so an undeclared card can be hand-edited with nothing noticing. The bijection now runs before that early return. Mutation-proven red-first (moving the call after the early return restores exit 0; reverted). | landed |
| `freshness-13` | freshness | the same tree with an EMPTY `entries: []` manifest ⇒ the same two ERRORs | **rk-nsex Tier A repair wave, BL4** — a genuinely different code path from `freshness-12`: the manifest exists and parses, so the gate walks a zero-length list and reports `checked 0/0` with no findings, which reads as a freshness gate that ran and found everything fine. Separately breakable, therefore separately fixtured. | landed |
| `freshness-14` | freshness | one correctly adopted, byte-identical card PLUS an undeclared second card file stating a strictly stronger theorem ⇒ ERROR `[card-undeclared]`, coverage still truthfully `checked 1/1` | **rk-nsex Tier A repair wave, BL4** — 'as does an undeclared stale card', the inverse direction of the bijection. The file is card-shaped, backed by no record, produced by no generator, and invisible to a gate that only looks at what the manifest names; an agent reading `refs/cards/` cannot tell it from the real one. Mutation-proven red-first (dropping the undeclared-file half restores `checked 1/1`, exit 0; reverted). | landed |
| `freshness-15` | freshness | declared `cards-v1` entry whose extraction record does not exist, card file present ⇒ ERROR `cannot be regenerated for verification`, counted in `checked` | **rk-nsex Tier A repair wave, BL4**, the third direction: the card is a real rendering and internally plausible, so a presence-only check passes it; what is missing is the AUTHORED record it claims to be a view of, so nothing can say what it should contain. Also the fixture that pins `cards-v1` findings as STRUCTURAL — unlike every other Gate 7 finding they must not demote in exploration (survival through `applyPhase` is proven in `test/render/cards.test.ts`, since the corpus harness never applies the phase matrix). | landed |
...
Totals: 5 config + 15 defs + 46 argument/linker + 43 refs + 24 provenance + 10 runs +
15 report-shards + 15 freshness + 28 reward = **201 fixtures** (reconciled 2026-08-10,
...
+1 refs-20 review repair 2026-08-14; +2 rk-r0j3, +2 rk-yic3 2026-08-14; +13 rk-nsex 2026-08-20/21)
across the gates named in
...
`refs-23`..`refs-32` (+10 over the then-pinned 176) are rk-nsex (Tier A): Gate 3's Check 11
(extraction-record verification) and Check 12 (the card->shard hash join), the mechanical half of
the repair the 2026-08-20 Tier A review demanded of the qPCP campaign plan
(`docs/reviews/2026-08-20-qpcp-plan-tierA-codex.md`, findings LB1/LB4/LB6). Nine red cases plus one
green control (`refs-32`), each red tree differing from the green one in exactly one place. These
are CLASS-DRIVEN, not incident-driven, in an unusual sense worth stating: the failure class is real
and named by a hostile review of a design, but no rk campaign has yet written an extraction record
at all, so no live repo has ever exhibited one of these states. They are preventive by
construction, and each is mutation-proven against the check that catches it rather than against a
historical run. The one honest gap is `refs-31`'s: the corpus harness never applies the phase
matrix, so survival-through-demotion is proven in unit tests, not here (see that fixture's row).
