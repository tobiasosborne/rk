...
| `notation-01` | notation | two tracked symbols (`\epsilon`, `\Delta`) used in a conjecture shard, neither registered ⇒ 2 structural ERRORs, `checked 0/2 symbols in 2 classes over 1 file` | **rk-5lzf** (Tier A, LB5/LB8): the qPCP literature writes `\Delta` for a promise gap in one paper and a spectral gap in another; a campaign that lets those tokens into its own prose unregistered is building on sentences that cannot be read back. This is LB8's "the trap list is mathematically incomplete", made mechanical. | landed |
...
| `notation-03` | notation | the same tracked-symbol usage with NO `conventionProfile` configured ⇒ WARN + `checked 0/0 (no profile configured)`, verdict pass | **rk-5lzf**: L2 verbatim — "Gates report coverage; a silent skip is a bug, full stop." The unconfigured state is stated, not merely absent, and is reported DIFFERENTLY from a configured-but-unusable profile: a campaign that believes it is being checked must not read the same output as one that knows it is not. | landed |
...
Totals: 7 config + 22 defs + 4 notation + 46 argument/linker + 22 refs + 24 provenance + 10 runs +
15 report-shards + 12 freshness + 28 reward = **190 fixtures** (reconciled 2026-08-10,
...
+2 the rk-5lzf Tier A repair wave `defs-21`, `defs-22` 2026-08-20) across the gates named in
