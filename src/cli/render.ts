...
export async function renderCommand(args: string[], out: Out, deps: RenderCommandDeps = {}): Promise<number> {
  ...
  // rk-nsex: `rk render cards` likewise — refs/cards/** from the extraction records, no
  // GraphDocument, no af/fr subprocess, no --out. Both subcommands adopt through the ONE manifest
  // writer in ./generated-manifest.ts.
  if (args[0] === "cards") {
    return renderCardsCommand(args.slice(1), out, (root, entryPath, generator) =>
      adoptGeneratedEntry(root, entryPath, generator, "rk render cards"));
  }
