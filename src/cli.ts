...
// subcommand) so future top-level commands (`rk check`, `rk doctor`) and future `rk refs`
// subcommands (`rk refs check`) slot in as one more registry entry, no dispatch-logic rework.
...
const defaultOut: Out = { log: (s) => console.log(s) };

type SubHandler = (args: string[], out: Out) => Promise<number>;

/** Pulls `--root <path>` out of argv, returning [remaining args, root]. Every refs subcommand
 * accepts it (defaults to cwd) so tests never depend on process.cwd(). */
function extractRoot(args: string[]): { rest: string[]; root: string } {
  const rest: string[] = [];
...
function extractFlag(args: string[], flag: string): { rest: string[]; value: string | undefined } {
  const rest: string[] = [];
...
async function refsDispatch(args: string[], out: Out): Promise<number> {
  const [sub, ...rest] = args;
