...
import { isMandatoryReview } from "./linker-parse";
...
// M3 blocker 7c: the mandatory-review board flag. Purely ADDITIVE and conditional on
// `isMandatoryReview(l.balloons)` — a shard with no balloon marks (`{count: 0, classifications:
// []}`, every pre-M3.6 shard and every existing golden corpus fixture) renders byte-IDENTICAL to
// before this wave, so Check 11's byte-for-byte match against committed argument/INDEX.md and
// argument/DAG.md mirrors (linker-17..20, linker-25) is unaffected. Text only — no new mermaid
// classDef (that block is emitted unconditionally by CLASSDEFS below; adding an entry there would
// change EVERY render's byte output, flagged or not).
const MANDATORY_REVIEW_MARK = "⚠MANDATORY-REVIEW";
...
export function renderIndex(lemmas: Lemma[]): string {
  ...
  for (const l of byIdAsc(lemmas)) {
    ...
    const flag = isMandatoryReview(l.balloons) ? ` ${MANDATORY_REVIEW_MARK}` : "";
    rows.push(`| \`${l.id}\` | ${l.kind ?? "?"} | ${l.status ?? "?"} | ${l.af} | ${l.owner ?? "-"} | ${c}${flag} |`);
  }
...
export function renderDag(lemmas: Lemma[]): string {
  ...
  const nodes = ll.map((l) => {
    const flag = isMandatoryReview(l.balloons) ? `<br/>${MANDATORY_REVIEW_MARK}` : "";
    return `  ${l.id}["${l.id}<br/>${l.status ?? ""}/${l.af}${flag}"]`;
  });
