// PURITY: pure — no fs/network/clock (L3). Gate 2 (linker) registry SCANNER: walks
// `argument/**/*.md` frontmatter (RECURSIVE) into `Lemma` records (shape owned by
// `linker-lemma.ts`, split out rk-c83) + parse-stage Findings (checks 1-5). Ground truth:
// docs/gate-contracts.md "Gate 2 — argument / linker" Inputs + Checks 1-5, ported from
// argument.py:106-148 (`_parse_frontmatter`/`parse_registry`) and `parse_routes`
...
//
// rk-c83 (hard-cap-280 split wave): the `Lemma` shape + its pure list/route grammar moved to
// `linker-lemma.ts`; the `definitions/*.md` id lookup moved to `linker-defs.ts`; the balloon
// mandatory-review predicate moved to `linker-balloon-review.ts`. All four are re-exported below
// unchanged so every existing import of `Lemma`/`parseRoutes`/`allDepIds`/`loadDefIds`/
// `isMandatoryReview` from "./linker-parse" keeps working without any edit at the call site.

import type { Finding } from "./framework";
import type { RepoSnapshot } from "./snapshot";
import { parseFrontmatter } from "./snapshot";
...
export { loadDefIds } from "./linker-defs";
export { isMandatoryReview } from "./linker-balloon-review";
