...
// fixture in its own `test()`/`expect()` for readable red output) and `bun run selftest` (which
// aggregates per-gate pass counts) — one runner, so the two can never silently disagree on what
// "passing the corpus" means. Fixes the guard-the-guards gap named in the 2026-07-18 M0.3
// milestone review (finding 6): corpus/README.md:13 always claimed selftest RUNS the corpus;
// before this file existed, `scripts/selftest.ts` only counted fixture directories.
...
import { loadGateConfig } from "./config-load";
import { unmatchedExpectations } from "./subset-match";
import type { ExpectedFinding } from "./subset-match";
import { GATE_DIRS, discoverAllFixtures } from "./corpus-discovery";
import type { GateDir } from "./corpus-discovery";
 ...
 * failure is appended to the returned `errors` list instead. */
export async function runFixture(corpusRoot: string, gate: GateDir, fixtureId: string): Promise<FixtureRunResult> {
  ...
  // rk-6vw (review finding 6): the OPTIONAL coverage expectation. Every gate in this suite emits
  // exactly one CoverageLine per run (docs/gate-contracts.md "Coverage line": "every gate emits
  // exactly one final line") — asserted here rather than assumed, so a future gate that starts
  // emitting more than one line fails loudly instead of silently comparing against the wrong
  // entry.
 ...
 * `discoverAllFixtures`'s own (alphabetical-within-gate) ordering, so output is stable. */
