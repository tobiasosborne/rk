...
describe("refsGate — checks 1-5", () => {
  ...
  test("check 5b: coverage still counts a malformed external in `total`/`failed`, never a silent drop", () => {
    ...
    expect(formatCoverageLine(result.coverage[0]!)).toBe(
      "checked refs: 0/2 externals byte-verified, 1 failed, 1 import-skipped, 0 no-quote-skipped; checked 0/0 shard citations; checked records: 0 L1 records / 0 reviewed-VALID / 0 anchors verified; 0/0 shard-record joins",
    );
  });
...
describe("refsGate — check 7 (the closed no-quote escape, I3)", () => {
  test("I3: a source naming a refs/ locus with NO extractable quote is an ERROR, not a WARN skip", () => {
    ...
    expect(formatCoverageLine(result.coverage[0]!)).toBe(
      "checked refs: 0/1 externals byte-verified, 1 failed, 0 import-skipped, 0 no-quote-skipped; checked 0/0 shard citations; checked records: 0 L1 records / 0 reviewed-VALID / 0 anchors verified; 0/0 shard-record joins",
    );
  });
  ...
  test("no refs/ locus at all ⇒ unchanged WARN skip_noquote (refs-05's golden case is untouched)", () => {
    ...
    expect(formatCoverageLine(result.coverage[0]!)).toBe(
      "checked refs: 0/1 externals byte-verified, 0 failed, 0 import-skipped, 1 no-quote-skipped; checked 0/0 shard citations; checked records: 0 L1 records / 0 reviewed-VALID / 0 anchors verified; 0/0 shard-record joins",
    );
  });
...
describe("refsGate — coverage line (four-way split, never merged)", () => {
  test("empty repo (day-1 state): 0/0, all-zero breakdown, still emitted (never a silent skip)", () => {
    ...
    expect(result.coverage).toHaveLength(1);
    expect(formatCoverageLine(result.coverage[0]!)).toBe(
      "checked refs: 0/0 externals byte-verified, 0 failed, 0 import-skipped, 0 no-quote-skipped; checked 0/0 shard citations; checked records: 0 L1 records / 0 reviewed-VALID / 0 anchors verified; 0/0 shard-record joins",
    );
  });
  ...
  test("one external of each of the four verdict classes: every count is independently visible", () => {
    ...
    expect(formatCoverageLine(c)).toBe(
      "checked refs: 1/4 externals byte-verified, 1 failed, 1 import-skipped, 1 no-quote-skipped; checked 0/0 shard citations; checked records: 0 L1 records / 0 reviewed-VALID / 0 anchors verified; 0/0 shard-record joins",
    );
  });
});
